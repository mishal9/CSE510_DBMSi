 ...
 */
public class ClBTIndexPage extends BTIndexPage{
	 ...
	 */
	public ClBTIndexPage(PageId pageno, int keyType) 
			throws IOException, 
	 ...
	 *@exception ConstructPageException error when BTIndexpage constructor
	 */
	public ClBTIndexPage(Page page, int keyType) 
			throws IOException, 
			ConstructPageException
	{
		super(page, keyType);
	 ...
	 *@exception ConstructPageException error when BTIndexpage constructor
	 */
	public ClBTIndexPage( int keyType) 
			throws IOException, 
			ConstructPageException
	{
		super(keyType);
   ...
   null if no space left.
			...
			throws IndexFullDeleteException 
	{
		...
		try {
			rid = firstRecord();
			entry = getFirst(rid);
			while ( rid.slotNo != getSlotCnt() ) {
//				System.out.println("rids "+rid.pageNo.pid+" "+rid.slotNo);
//				System.out.println("entry value "+entry.data.toString());
				entry = getNext(rid);
			}
			entry = getFirst(rid);
//			System.out.println("key deletion is " + entry.key.toString());
			if (entry == null) 
...
//			System.out.println("Entry deletion nlo "+entry.key.toString());
//			System.out.println("Deletion nlo "+rid.pageNo.pid+" "+rid.slotNo);
	 ...
	 *@return  RID of the record deleted. Can not return null.
	 */
	public RID deleteKeyentry(KeyDataEntry delete_entry) 
			throws IndexFullDeleteException 
	{
		KeyDataEntry  entry;
		RID rid=new RID(); 
//		System.out.println("Entry deletion nlo "+delete_entry.data.toString()+" key "+delete_entry.key.toString());
...
//			System.out.println("key deletion is " + entry.key.toString());
			...
			if ( BT.keyCompare(delete_entry.key, entry.key)<0 )  
				//it is supposed to not smaller than first key
				throw new IndexFullDeleteException(null, "First key is bigger");


			while ( entry != null ) {
				if ( ((IndexData)delete_entry.data).getData().pid == ((IndexData)entry.data).getData().pid ) {
					break;
				}
				entry = getNext(rid);
			}
			...
			if (entry == null) rid.slotNo--;
//			else if (BT.keyCompare(delete_entry.key, entry.key) != 0)
//				rid.slotNo--; // we want to delete the previous key
//			System.out.println("Entry deletion nlo "+entry.key.toString());
//			System.out.println("Deletion nlo "+rid.pageNo.pid+" "+rid.slotNo);
			deleteSortedRecord(rid);
	 ...
	 */
	public PageId getPageNoByKeyCustom(KeyClass key, short keytype) 
			throws IndexSearchException         
	{
		...
		try {
			...
			if ( (getPrevPage()).pid != INVALID_PAGE ) {
				return getPrevPage();
			}
...
//				System.out.println("Returning from custom "+((IndexData)entry.data).getData());
				return ((IndexData)entry.data).getData();
			}
			return (new PageId(INVALID_PAGE));
		} 
		catch (Exception e) {
	 ...
	 *@exception IndexSearchException Index search failed;
	 */
	public PageId getPageNoByKey(KeyClass key, short keytype) 
			throws IndexSearchException         
	{
		KeyDataEntry entry;
		int i;
		...
		try {
			...
			for (i=getSlotCnt()-1; i >= 0; i--) {
				...
				if (BT.keyCompare(key, entry.key) >= 0)
				{
					if ( BT.keyCompare(key, entry.key) == 0 ) {
						...
						PageId min_page = ((IndexData)entry.data).getData();
						Page page = new Page();
						SystemDefs.JavabaseBM.pinPage(min_page, page, false);
						BTSortedPage sortPage=new BTSortedPage(page, keytype);
						RID last_rid = new RID();
						KeyDataEntry last_entry = sortPage.getLast(last_rid);
						SystemDefs.JavabaseBM.unpinPage(min_page, false);
						if ( BT.keyCompare(last_entry.key, key) >= 0 ) {
							min_i = i;
						}
						else {
							break;
			...
			if ( min_i != -1 ) {
				entry= BT.getEntryFromBytes( getpage(),getSlotOffset(min_i), 
						getSlotLength(min_i), keyType, NodeType.INDEX);
				return ((IndexData)entry.data).getData();
			}
			if ( (getPrevPage()).pid != INVALID_PAGE ) {
				return getPrevPage();
...
//				System.out.println("Returning from custom "+((IndexData)entry.data).getData());
			...
			else return (new PageId(INVALID_PAGE));
		} 
		catch (Exception e) {
			throw new IndexSearchException(e, "Get entry failed");
		}   
	...
	public int  getSibling(KeyClass key, PageId pageNo, PageId curr_page)
			throws IndexFullDeleteException
	{
		...
		try {
			if (getSlotCnt() == 0) // there is no sibling 
				return 0;
//			System.out.println("slots "+getSlotCnt());
			...
			for (i=getSlotCnt()-1; i >= 0; i--) {
				entry=BT.getEntryFromBytes(getpage(), getSlotOffset(i),
						getSlotLength(i), keyType , NodeType.INDEX);
//				System.out.print("entry no "+i+" "+((IndexData)entry.data).getData().pid);
...
//						System.out.println("Sibling key "+entry.key.toString());
//						
//						System.out.println("Sibling "+pageNo.pid);
						return -1; //left sibling
					}
					else {
						pageNo.pid = getLeftLink().pid;
						System.out.println("Sibling1 "+pageNo.pid);
			...
			entry=BT.getEntryFromBytes(getpage(), getSlotOffset(0),
					getSlotLength(0), keyType, NodeType.INDEX);    
			pageNo.pid=  getNextPage().pid;
//			System.out.println("Sibling2 "+pageNo.pid);
			if ( pageNo.pid != INVALID_PAGE ) {
				return 1;
			}
			return 0;  //right sibling
		}
		catch (Exception e) {
	...
	public int  getSiblingCustom(KeyClass key, PageId pageNo, PageId curr_page)
			throws IndexFullDeleteException
	{
		try {
			if (getSlotCnt() == 0) // there is no sibling 
			...
			for (i=getSlotCnt()-1; i >= 0; i--) {
				entry=BT.getEntryFromBytes(getpage(), getSlotOffset(i),
							getSlotLength(i), keyType , NodeType.INDEX);
				if ( ((IndexData)entry.data).getData().pid == curr_page.pid ) {
					break;
				}
			}
			if ( ( i<getSlotCnt() ) && ( i >= 0) ) {
...
//					return 0;
	 ...
	 */
	public boolean redistribute(ClBTIndexPage indexPage, ClBTIndexPage parentIndexPage,
